...
do
    if [ ${number} -lt 10 ]
    then 
        bash fb-float.sh 0${number}
    else
        bash fb-float.sh ${number}
    fi
done
...
do
    if [ ${number} -lt 10 ]
    then
        nvprof --print-gpu-trace --normalized-time-unit ms ./k_fb_0${number} ${tccg_problem_sizes[$number]} |& tee output_fb_0${number}.txt
    else
        nvprof --print-gpu-trace --normalized-time-unit ms ./k_fb_${number} ${tccg_problem_sizes[$number]} |& tee output_fb_${number}.txt
    fi
done
