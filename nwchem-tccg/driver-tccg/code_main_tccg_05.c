...
int main(int argc, char** argv)
{
	...
	int size_B;
	int size_internal;
    long long int tmp_num = (long long int)(size_idx_a * size_idx_b * size_idx_c * size_idx_d * size_idx_e * 2);

	// abcd-ebad-ce
    // t3 [a,16,b,16,c,16,d,16] += sum(e,16) * t2 [e,b,a,d] * v2 [c,e];
	size_internal 	= size_idx_e;
	size_C = size_idx_a * size_idx_b * size_idx_c * size_idx_d;
	size_A = size_idx_e * size_idx_b * size_idx_a * size_idx_d;
    ...
    // Run the Kernels
	initmemmodule_();
	tccg_05_cuda(size_idx_a, size_idx_b, size_idx_c, size_idx_d, size_idx_e, host_C, host_A, host_B);
...
void post_Correctness(double* h_C, double* h_C_chk, 
				    ...
				    int size_idx_a, int size_idx_b, int size_idx_c, int size_idx_d, int size_idx_e)
{
    // abcd-ebad-ce
    // t3 [a,16,b,16,c,16,d,16] += sum(e,16) * t2 [e,b,a,d] * v2 [c,e];
    ...
    for (idx_d = 0; idx_d < size_idx_d; idx_d++)
    {
        ...
        /*
            if (tmp_r_idx == 256)
            {
                printf ("[%2d] t2: %f, v2: %f >>> %f\n", idx_e, h_A[idx_e + (idx_b + (idx_a + (idx_d) * size_idx_a) * size_idx_b) * size_idx_e], 
                h_B[idx_c + (idx_e) * size_idx_c], h_C_chk[tmp_r_idx]);
            }
        */
            ops++;
